...
const N8N_WEBHOOK = '[https://narodocnt.online:5678/webhook/register](https://narodocnt.online:5678/webhook/register)';
...
container.innerHTML = `     <button id="loginBtn" class="login-btn">Log in ▾</button>     <div id="loginDropdown" class="dropdown">       <a id="menuLogin">Увійти</a>       <a id="menuEmail">через e-mail</a>       <a id="menuGoogle">через Google</a>       <a id="menuFacebook">через Facebook</a>     </div>
  `;
document.body.appendChild(container);

// Модалки
const modalsHTML = `     <div id="modalLogin" class="backdrop">       <div class="modal">         <h2>Увійти</h2>         <div class="form-row"><label>Логін</label><input id="login_login" type="text"/></div>         <div class="form-row"><label>Пароль</label><input id="login_pass" type="password"/></div>         <div class="actions">           <button class="btn cancel" onclick="closeAll()">Cancel</button>           <button class="btn ok" onclick="submitLogin()">OK</button>         </div>       </div>     </div>     <div id="modalEmail" class="backdrop">       <div class="modal">         <h2>Реєстрація через e-mail</h2>         <div class="form-row"><label>Прізвище</label><input id="reg_lastname" type="text"/></div>         <div class="form-row"><label>Ім'я</label><input id="reg_firstname" type="text"/></div>         <div class="form-row"><label>По-батькові</label><input id="reg_patronymic" type="text"/></div>         <div class="form-row"><label>E-mail</label><input id="reg_email" type="email"/></div>         <div class="form-row"><label>Пароль</label><input id="reg_password" type="password"/></div>         <div class="social-row">           <button class="social-btn social-google" id="regGoogle">Sign up with Google</button>           <button class="social-btn social-fb" id="regFb">Sign up with Facebook</button>         </div>         <div class="actions">           <button class="btn cancel" onclick="closeAll()">Cancel</button>           <button class="btn ok" onclick="submitEmail()">OK</button>         </div>       </div>     </div>
  `;
...
const url = `https://www.facebook.com/v16.0/dialog/oauth?client_id=${fbId}&redirect_uri=${encodeURIComponent(fbRedirect)}&response_type=token&scope=email`;
